...
kbmf1mkl1k_supervised_regression_variational_train <- function(Kx, Kz, Y, parameters) {
  ...
  for (iter in 1:parameters$iteration) {
    ...
    # update Hx
    Hx$covariance <- chol2inv(chol(diag(1 / sigmah^2, R, R) + (tcrossprod(Gz$mean, Gz$mean) + Nz * Gz$covariance) / sigmay^2))
    Hx$mean <- tcrossprod(Gz$mean, Y) / sigmay^2
    for (m in 1:Px) {
    ...
    # update Gz
    Gz$covariance <- chol2inv(chol(diag(1 / sigmag^2, R, R) + (tcrossprod(Hx$mean, Hx$mean) + Nx * Hx$covariance) / sigmay^2))
    Gz$mean <- Gz$covariance %*% (crossprod(Az$mean, Kz) / sigmag^2 + Hx$mean %*% Y / sigmay^2)
  }
