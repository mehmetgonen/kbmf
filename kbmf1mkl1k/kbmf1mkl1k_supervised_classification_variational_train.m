...
function state = kbmf1mkl1k_supervised_classification_variational_train(Kx, Kz, Y, parameters)
    ...
    for iter = 1:parameters.iteration
        ...
        %%%% update Hx
        Hx.covariance = (eye(R, R) / sigmah^2 + Gz.mean * Gz.mean' + Nz * Gz.covariance) \ eye(R, R);
        Hx.mean = Gz.mean * F.mean';
        for m = 1:Px
        ...
        %%%% update Gz
        Gz.covariance = (eye(R, R) / sigmag^2 + Hx.mean * Hx.mean' + Nx * Hx.covariance) \ eye(R, R);
        Gz.mean = Gz.covariance * (Az.mean' * Kz / sigmag^2 + Hx.mean * F.mean);
