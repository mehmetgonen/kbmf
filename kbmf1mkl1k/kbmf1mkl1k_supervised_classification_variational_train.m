function state = kbmf1mkl1k_supervised_classification_variational_train(Kx, Kz, Y, parameters)
    ...
    for iter = 1:parameters.iteration
        ...
        %%%% update Hx
        Hx.sigma = (eye(R, R) / sigma_h^2 + Gz.mu * Gz.mu' + Nz * Gz.sigma) \ eye(R, R);
        Hx.mu = Gz.mu * F.mu';
        for m = 1:Px
        ...
        %%%% update Gz
        Gz.sigma = (eye(R, R) / sigma_g^2 + Hx.mu * Hx.mu' + Nx * Hx.sigma) \ eye(R, R);
        Gz.mu = Gz.sigma * (Az.mu' * Kz / sigma_g^2 + Hx.mu * F.mu);
