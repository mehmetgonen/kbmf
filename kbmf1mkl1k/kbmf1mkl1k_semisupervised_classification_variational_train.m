function state = kbmf1mkl1k_semisupervised_classification_variational_train(Kx, Kz, Y, parameters)
    ...
    for iter = 1:parameters.iteration
        ...
        for i = 1:Nx
            indices = ~isnan(Y(i, :));
            Hx.sigma(:, :, i) = (eye(R, R) / sigma_h^2 + Gz.mu(:, indices) * Gz.mu(:, indices)' + sum(Gz.sigma(:, :, indices), 3)) \ eye(R, R);
            Hx.mu(:, i) = Gz.mu(:, indices) * F.mu(i, indices)';
            for m = 1:Px
        ...
        for j = 1:Nz
            indices = ~isnan(Y(:, j));
            Gz.sigma(:, :, j) = (eye(R, R) / sigma_g^2 + Hx.mu(:, indices) * Hx.mu(:, indices)' + sum(Hx.sigma(:, :, indices), 3)) \ eye(R, R);
            Gz.mu(:, j) = Gz.sigma(:, :, j) * (Az.mu' * Kz(:, j) / sigma_g^2 + Hx.mu(:, indices) * F.mu(indices, j));
        end
