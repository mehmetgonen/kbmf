kbmf1mkl1k_semisupervised_regression_variational_train <- function(Kx, Kz, Y, parameters) {
  ...
  for (iter in 1:parameters$iteration) {
    ...
    for (i in 1:Nx) {
      ...
      Hx$sigma[,,i] <- chol2inv(chol(diag(1 / sigma_h^2, R, R) + (tcrossprod(Gz$mu[,indices, drop = FALSE], Gz$mu[,indices, drop = FALSE]) + apply(Gz$sigma[,,indices, drop = FALSE], 1:2, sum)) / sigma_y^2))
      Hx$mu[,i] <- tcrossprod(Gz$mu[,indices, drop = FALSE], Y[i, indices, drop = FALSE]) / sigma_y^2
      for (m in 1:Px) {
    ...
    for (j in 1:Nz) {
      ...
      Gz$sigma[,,j] <- chol2inv(chol(diag(1 / sigma_g^2, R, R) + (tcrossprod(Hx$mu[,indices, drop = FALSE], Hx$mu[,indices, drop = FALSE]) + apply(Hx$sigma[,,indices, drop = FALSE], 1:2, sum)) / sigma_y^2))
      Gz$mu[,j] <- Gz$sigma[,,j] %*% (crossprod(Az$mu, Kz[,j]) / sigma_g^2 + Hx$mu[,indices, drop = FALSE] %*% Y[indices, j, drop = FALSE] / sigma_y^2)
    }
  }
