...
kbmf1k1mkl_supervised_classification_variational_train <- function(Kx, Kz, Y, parameters) {
  ...
  for (iter in 1:parameters$iteration) {
    ...
    # update Gx
    Gx$covariance <- chol2inv(chol(diag(1 / sigmag^2, R, R) + tcrossprod(Hz$mean, Hz$mean) + Nz * Hz$covariance))
    Gx$mean <- Gx$covariance %*% (crossprod(Ax$mean, Kx) / sigmag^2 + tcrossprod(Hz$mean, F$mean))
    ...
    # update Hz
    Hz$covariance <- chol2inv(chol(diag(1 / sigmah^2, R, R) + tcrossprod(Gx$mean, Gx$mean) + Nx * Gx$covariance))
