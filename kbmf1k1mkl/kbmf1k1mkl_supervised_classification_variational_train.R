kbmf1k1mkl_supervised_classification_variational_train <- function(Kx, Kz, Y, parameters) {
  ...
  for (iter in 1:parameters$iteration) {
    ...
    # update Gx
    Gx$sigma <- chol2inv(chol(diag(1 / sigma_g^2, R, R) + tcrossprod(Hz$mu, Hz$mu) + Nz * Hz$sigma))
    Gx$mu <- Gx$sigma %*% (crossprod(Ax$mu, Kx) / sigma_g^2 + tcrossprod(Hz$mu, F$mu))
    ...
    # update Hz
    Hz$sigma <- chol2inv(chol(diag(1 / sigma_h^2, R, R) + tcrossprod(Gx$mu, Gx$mu) + Nx * Gx$sigma))
    Hz$mu <- Gx$mu %*% F$mu
    for (n in 1:Pz) {
