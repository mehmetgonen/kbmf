kbmf1k1mkl_semisupervised_classification_variational_train <- function(Kx, Kz, Y, parameters) {
  ...
  for (iter in 1:parameters$iteration) {
    ...
    for (i in 1:Nx) {
      ...
      Gx$sigma[,,i] <- chol2inv(chol(diag(1 / sigma_g^2, R, R) + tcrossprod(Hz$mu[,indices, drop = FALSE], Hz$mu[,indices, drop = FALSE]) + apply(Hz$sigma[,,indices, drop = FALSE], 1:2, sum)))
      Gx$mu[,i] <- Gx$sigma[,,i] %*% (crossprod(Ax$mu, Kx[,i]) / sigma_g^2 + tcrossprod(Hz$mu[,indices, drop = FALSE], F$mu[i, indices, drop = FALSE]))
    }
    ...
    for (j in 1:Nz) {
      ...
      Hz$sigma[,,j] <- chol2inv(chol(diag(1 / sigma_h^2, R, R) + tcrossprod(Gx$mu[,indices, drop = FALSE], Gx$mu[,indices, drop = FALSE]) + apply(Gx$sigma[,,indices, drop = FALSE], 1:2, sum)))
      Hz$mu[,j] <- Gx$mu[,indices, drop = FALSE] %*% F$mu[indices, j, drop = FALSE]
      for (n in 1:Pz) {
