function state = kbmf1k1mkl_supervised_regression_variational_train(Kx, Kz, Y, parameters)
    ...
    for iter = 1:parameters.iteration
        ...
        %%%% update Gx
        Gx.sigma = (eye(R, R) / sigma_g^2 + (Hz.mu * Hz.mu' + Nz * Hz.sigma) / sigma_y^2) \ eye(R, R);
        Gx.mu = Gx.sigma * (Ax.mu' * Kx / sigma_g^2 + Hz.mu * Y' / sigma_y^2);
        ...
        %%%% update Hz
        Hz.sigma = (eye(R, R) / sigma_h^2 + (Gx.mu * Gx.mu' + Nx * Gx.sigma) / sigma_y^2) \ eye(R, R);
