...
function state = kbmf1mkl1mkl_supervised_classification_variational_train(Kx, Kz, Y, parameters)
    ...
    for iter = 1:parameters.iteration
        ...
        %%%% update Hx
        Hx.covariance = (eye(R, R) / sigmah^2 + Hz.mean * Hz.mean' + Nz * Hz.covariance) \ eye(R, R);
        Hx.mean = Hz.mean * F.mean';
        for m = 1:Px
        ...
        %%%% update Hz
        Hz.covariance = (eye(R, R) / sigmah^2 + Hx.mean * Hx.mean' + Nx * Hx.covariance) \ eye(R, R);
