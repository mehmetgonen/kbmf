...
kbmf1mkl1mkl_semisupervised_regression_variational_train <- function(Kx, Kz, Y, parameters) {
  ...
  for (iter in 1:parameters$iteration) {
    ...
    for (i in 1:Nx) {
      ...
      Hx$covariance[,,i] <- chol2inv(chol(diag(1 / sigmah^2, R, R) + (tcrossprod(Hz$mean[,indices], Hz$mean[,indices]) + apply(Hz$covariance[,,indices], 1:2, sum)) / sigmay^2))
      Hx$mean[,i] <- tcrossprod(Hz$mean[,indices], Y[i, indices, drop = FALSE]) / sigmay^2
      for (m in 1:Px) {
    ...
    for (j in 1:Nz) {
      ...
      Hz$covariance[,,j] <- chol2inv(chol(diag(1 / sigmah^2, R, R) + (tcrossprod(Hx$mean[,indices], Hx$mean[,indices]) + apply(Hx$covariance[,,indices], 1:2, sum)) / sigmay^2))
