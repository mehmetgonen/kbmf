...
function state = kbmf1mkl1mkl_semisupervised_classification_variational_train(Kx, Kz, Y, parameters)
    ...
    for iter = 1:parameters.iteration
        ...
        for i = 1:Nx
            indices = ~isnan(Y(i, :));
            Hx.covariance(:, :, i) = (eye(R, R) / sigmah^2 + Hz.mean(:, indices) * Hz.mean(:, indices)' + sum(Hz.covariance(:, :, indices), 3)) \ eye(R, R);
            Hx.mean(:, i) = Hz.mean(:, indices) * F.mean(i, indices)';
            for m = 1:Px
        ...
        for j = 1:Nz
            indices = ~isnan(Y(:, j));
            Hz.covariance(:, :, j) = (eye(R, R) / sigmah^2 + Hx.mean(:, indices) * Hx.mean(:, indices)' + sum(Hx.covariance(:, :, indices), 3)) \ eye(R, R);
