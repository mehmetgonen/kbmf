...
kbmf1mkl1mkl_supervised_regression_variational_train <- function(Kx, Kz, Y, parameters) {
  ...
  for (iter in 1:parameters$iteration) {
    ...
    # update Hx
    Hx$covariance <- chol2inv(chol(diag(1 / sigmah^2, R, R) + (tcrossprod(Hz$mean, Hz$mean) + Nz * Hz$covariance) / sigmay^2))
    Hx$mean <- tcrossprod(Hz$mean, Y) / sigmay^2
    for (m in 1:Px) {
    ...
    # update Hz
    Hz$covariance <- chol2inv(chol(diag(1 / sigmah^2, R, R) + (tcrossprod(Hx$mean, Hx$mean) + Nx * Hx$covariance) / sigmay^2))
