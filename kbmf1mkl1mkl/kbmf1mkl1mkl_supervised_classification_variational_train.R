...
kbmf1mkl1mkl_supervised_classification_variational_train <- function(Kx, Kz, Y, parameters) {
  ...
  for (iter in 1:parameters$iteration) {
    ...
    # update Hx
    Hx$covariance <- chol2inv(chol(diag(1 / sigmah^2, R, R) + tcrossprod(Hz$mean, Hz$mean) + Nz * Hz$covariance))
    Hx$mean <- tcrossprod(Hz$mean, F$mean)
    for (m in 1:Px) {
    ...
    # update Hz
    Hz$covariance <- chol2inv(chol(diag(1 / sigmah^2, R, R) + tcrossprod(Hx$mean, Hx$mean) + Nx * Hx$covariance))
    Hz$mean <- Hx$mean %*% F$mean
    for (n in 1:Pz) {
    ...
    # update F
    output <- crossprod(Hx$mean, Hz$mean)
